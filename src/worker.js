...
async function handleSearch(request, env, ctx) {
  ...
  // Build API body with all supported filters
  const apiBody = {
    origin: data.origin,
  ...
  if (data.infants) apiBody.infants = data.infants; // backward compat
...
async function handleBookingLink(request, env, ctx) {
  ...
  try {
    ...
    // Return full booking options, plus a convenience first URL
    let bookingUrl = null;
    let allLinks = [];
...
async function handlePriceTrack(request, env, ctx) {
  ...
  try {
    ...
    if (env.ENQUIRIES) {
      ctx.waitUntil(env.ENQUIRIES.put(trackKey, JSON.stringify(record), { expirationTtl: 7776000 }));
    }
...
async function handleAdminTicket(request, env, ctx) {
  ...
  try {
    await env.DB.prepare(
      "UPDATE bookings SET pnr = ?, ticket_number = ?, ticket_pdf = ?, status = 'confirmed', payment_status = 'verified', updated_at = datetime('now') WHERE id = ?"
    ).bind(data.pnr, data.ticket_number || "", data.ticket_pdf || "", data.booking_id).run();
    const booking = await env.DB.prepare("SELECT * FROM bookings WHERE id = ?").bind(data.booking_id).first();
...
async function sendNotificationEmail(env, data, isRoundTrip) {
  try {
    ...
    await fetch("https://api.web3forms.com/submit", {
      ...
      body: JSON.stringify({ access_key: env.WEB3FORMS_KEY, subject: "New Flight Search - " + data.origin + " to " + data.destination, from_name: "TrustedFare Website", to: "gm@trustedfare.com", replyto: data.customerEmail || "noreply@trustedfare.com", message: messageBody }),
    });
...
async function sendBookingEmail(env, data, bookingId) {
  try {
    ...
    await fetch("https://api.web3forms.com/submit", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ access_key: env.WEB3FORMS_KEY, subject: "New Booking " + bookingId + " - " + data.origin + " to " + data.destination, from_name: "TrustedFare Website", to: "gm@trustedfare.com", replyto: data.customer_email || "noreply@trustedfare.com", message: messageBody }),
    });
...
async function sendPaymentEmail(env, booking, utr) {
  try {
    ...
    await fetch("https://api.web3forms.com/submit", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ access_key: env.WEB3FORMS_KEY, subject: "Payment Submitted for " + booking.id, from_name: "TrustedFare Website", to: "gm@trustedfare.com", replyto: booking.customer_email || "noreply@trustedfare.com", message: messageBody }),
    });
...
async function sendTicketEmail(env, booking) {
  try {
    ...
    await fetch("https://api.web3forms.com/submit", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ access_key: env.WEB3FORMS_KEY, subject: "Ticket Confirmed - " + booking.id + " - " + booking.origin + " to " + booking.destination, from_name: "TrustedFare Website", to: booking.customer_email, replyto: "gm@trustedfare.com", message: messageBody }),
    });
