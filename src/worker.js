...
async function handleSearch(request, env, ctx) {
  ...
  if (env.SEND_EMAIL) { ctx.waitUntil(sendEnquiryEmail(env, data, isRoundTrip)); }
...
async function handleEnquiry(request, env, ctx) {
  ...
  if (env.SEND_EMAIL) { ctx.waitUntil(sendEnquiryEmail(env, data, !!data.return_date)); }
  return jsonResponse({ success: true, message: "Enquiry submitted.", enquiryId }, 200, cors);
}

async function sendEnquiryEmail(env, data, isRoundTrip) {
  try {
    const tp = [];
    ...
    const subject = "New Flight Search - " + data.origin + " to " + data.destination;
    const textBody = ["NEW FLIGHT SEARCH - TRUSTEDFARE", "================================", "", "Route: " + data.origin + " to " + data.destination, "Departure: " + data.departure_date, isRoundTrip ? "Return: " + data.return_date : "One-way", "Cabin: " + (data.cabin_class || "economy"), "Travelers: " + (tp.join(", ") || "1 Adult"), "", "Phone: " + (data.customerMobile || "N/A"), "Email: " + (data.customerEmail || "N/A"), "", "Submitted: " + (data.submittedAt || new Date().toISOString()), "================================"].join("\n");
    const message = new EmailMessage("noreply@trustedfare.com", data.notificationEmail || "gm@trustedfare.com", buildMimeEmail({ from: "noreply@trustedfare.com", to: data.notificationEmail || "gm@trustedfare.com", subject, text: textBody }));
...
function buildMimeEmail(opts) {
  ...
  const boundary = "----=_TrustedFare_" + Date.now();
  return ["From: TrustedFare <" + opts.from + ">", "To: <" + opts.to + ">", "Subject: " + opts.subject, "Date: " + date, "MIME-Version: 1.0", "Content-Type: multipart/alternative; boundary=\"" + boundary + "\"", "", "--" + boundary, "Content-Type: text/plain; charset=utf-8", "Content-Transfer-Encoding: 7bit", "", opts.text, "", "--" + boundary + "--"].join("\r\n");
}
